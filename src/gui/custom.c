...
void confirm_btn_event_cb(lv_event_t *e) {
    ...
    lv_obj_add_flag(guider_ui.network_setting_overlay, LV_OBJ_FLAG_HIDDEN);
    connect_wifi(guider_ui.clicked_wifi_ssid, guider_ui.input_wifi_password);
}

void overlay_event_cb(lv_event_t *e) {
    // 如果当前键盘是显示状态，就关闭键盘
    if (!lv_obj_has_flag(guider_ui.network_setting_keyboard, LV_OBJ_FLAG_HIDDEN)) {
        lv_obj_add_flag(guider_ui.network_setting_keyboard, LV_OBJ_FLAG_HIDDEN);
    } else {  // 如果键盘没有显示，就关闭联网窗口
        lv_obj_add_flag(guider_ui.network_setting_connect_wifi_window, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(guider_ui.network_setting_overlay, LV_OBJ_FLAG_HIDDEN);
    }
}
...
void keyboard_ready_event_cb(lv_event_t *e) {
    guider_ui.input_wifi_password = strdup(lv_textarea_get_text(guider_ui.network_setting_wifi_password_textarea));
    lv_obj_add_flag(guider_ui.network_setting_keyboard, LV_OBJ_FLAG_HIDDEN);
    lv_obj_clear_state(guider_ui.network_setting_wifi_password_textarea, LV_STATE_FOCUSED);
}

void network_setting_custom_init(lv_ui *ui) {
    // 在屏幕上创建一个透明遮罩层
    ui->network_setting_overlay = lv_obj_create(guider_ui.network_setting);
    lv_obj_remove_style_all(ui->network_setting_overlay);
    lv_obj_set_size(ui->network_setting_overlay, LV_PCT(100), LV_PCT(100));
    lv_obj_set_style_bg_opa(ui->network_setting_overlay, LV_OPA_TRANSP, 0); // 完全透明
    lv_obj_add_flag(ui->network_setting_overlay, LV_OBJ_FLAG_CLICKABLE); // 可点击
    lv_obj_add_flag(ui->network_setting_overlay, LV_OBJ_FLAG_HIDDEN);  // 默认隐藏
    lv_obj_add_event_cb(ui->network_setting_overlay, overlay_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_move_foreground(ui->network_setting_connect_wifi_window); // 窗口放到透明遮罩层上方
