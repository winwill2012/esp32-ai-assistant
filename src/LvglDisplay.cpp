...
void LvglDisplay::updateRecordingButtonImage(const void *img) {
    if (xSemaphoreTakeRecursive(lvglUpdateLock, portMAX_DELAY) == pdTRUE) {
        lv_imgbtn_set_src(guider_ui.screen_main_imgbtn_mic, LV_IMGBTN_STATE_RELEASED,
                          NULL, img, NULL);
        xSemaphoreGiveRecursive(lvglUpdateLock);
    }
}
...
void LvglDisplay::updateRecordingState(bool active) {
    if (xSemaphoreTakeRecursive(lvglUpdateLock, portMAX_DELAY) == pdTRUE) {
        if (active) {
            lv_obj_set_style_bg_color(guider_ui.screen_main_btn_recording_state, lv_color_make(0, 255, 0), 0);
        } else {
            lv_obj_set_style_bg_color(guider_ui.screen_main_btn_recording_state, lv_color_make(153, 153, 153), 0);
        }
        xSemaphoreGiveRecursive(lvglUpdateLock);
    }
}
