...
[[noreturn]] void consumeBufferedData(void *arg) {
    ...
    while (true) {
        ...
        if (buffer == nullptr) {
            if (!firstPacket) {
                application->getSTTInstance()->recognize(lastPacketBuff.data(), lastPacketBuff.size(),
                                                         firstPacket, true);
                firstPacket = true;
            }
        } else {
            application->getSTTInstance()->recognize(buffer, bytes_read, firstPacket, false);
            if (firstPacket) {
                firstPacket = false;
            }
            vRingbufferReturnItem(application->recordingManager()->getRingBuffer(), (void *) buffer);
        }
        vTaskDelay(1);
    }
}
    ...
    while (true) {
        ...
        if (err == ESP_OK) {
            if (calculateSoundRMS(buffer, bytesRead) > Settings::getRecordingRmsThreshold()) {
                ...
                // 再次说话时，终止状态清除
                Application::getInstance()->getAudioPlayer()->interrupt(false);
                Application::getInstance()->getTTSInstance()->interrupt(false);
                Application::getInstance()->getLlmAgentInstance()->interrupt(false);
